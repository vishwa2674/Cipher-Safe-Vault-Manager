...
class PasswordManager:
    ...
    @staticmethod
    def pass_gen(size=14, use_uppercase=True, use_numbers=True, use_special_chars=True):
        ...
        password = ''.join(secrets.choice(chars) for _ in range(size))
        return password

    @staticmethod
    def suggest_password(size=14, use_uppercase=True, use_numbers=True, use_special_chars=True):
        password = PasswordManager.pass_gen(size, use_uppercase, use_numbers, use_special_chars)
        return password
    ...
    def add_to_db(self, serv, mail, pwd):
        ...
        print(Fore.GREEN + "Success: Entry Successfully Added!" + Style.RESET_ALL)
    ...
    def update_db(self, index, pwd):
        try:
            ...
            print(Fore.YELLOW + "Info: Entry Successfully updated" + Style.RESET_ALL)
    ...
    while main_screen:
        ...
        if main_scr_choice == 'a' or main_scr_choice == 'add':
            ...
            while add_screen:
                ...
                if genPass == 'y':
                    password = user.pwdmngr.suggest_password()
                    print("\nPassword Suggestion: ", password)
                    use_suggested_password = input("\nDo you want to use this password?(Y/N)").lower()
                    if use_suggested_password == 'y':
                        tempAddPwd = password
                    else:
                        while True:
                            try:
...
def login_sign_up():
    ...
    main_scr_options = ['l', 'login', 's', 'signup','fp','forgotpassword', 'q', 'quit']
    ...
    while main_scr:
        ...
        print("\nPress : (L)ogin, (S)ign-up, (FP)Forgot Password, (Q)uit  \n")
        main_scr_choice = input("Enter your choice : ").lower()
        if main_scr_choice not in main_scr_options:
            print("\nEnter a valid choice!\n")
            continue
        ...
        if main_scr_choice == 'l' or main_scr_choice == 'login':
                    ...
                    if email != '' and pwd != '' and cnPwd != '':
                        if pwd == cnPwd:
                            try:
                                ...
                                lgn = auth.sign_in_with_email_and_password(email,pwd)
                                auth.send_email_verification(lgn['idToken'])
            ...
            main_scr = False
            exit()
        elif main_scr_choice == 'fp' or main_scr_choice == 'forgotpassword':
            forgot_password_scr = True
            main_scr = False
    
            while forgot_password_scr:
